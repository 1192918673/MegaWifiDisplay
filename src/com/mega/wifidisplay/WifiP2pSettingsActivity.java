...
import com.mega.wifidisplay.util.RarpUtils;
...
public class WifiP2pSettingsActivity extends PreferenceActivity {
    ...
    private static final long ARP_RETRY_INTERVAL = 1000;
    ...
    private class RetryStartWfdSinkRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mGroup.isGroupOwner()) {
                ...
                String ifName = mGroup.getInterface();
                sourceIp = RarpUtils.execRarp(ifName, sourceMac);
                if (sourceIp == null) {
                    if (mRetryCount++ < MAX_ARP_RETRY_COUNT) {
                        Log.w(TAG, "Can't find WFD source ip. Retry " + mRetryCount);
                        mHandler.postDelayed(this, ARP_RETRY_INTERVAL);
                    } else {
                        Log.e(TAG, "Still can't find WFD source ip. Give up!");
                    }
                    return;
                }
            } else {
            ...
            startWifiDisplaySinkActivity(sourceIp, controlPort);
    ...
    private void startWifiDisplaySinkActivity(String sourceIp, int sourcePort) {
        ...
        itent.putExtra(WfdConstants.SOURCE_HOST, sourceIp);
        itent.putExtra(WfdConstants.SOURCE_PORT, sourcePort);
        startActivity(itent);
        finish();
    }
